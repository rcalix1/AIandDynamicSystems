...
plt.plot(best_traj[:,0,0].cpu(),label="Susceptible")

plt.plot(best_traj[:,0,1].cpu(),label="Infected")

plt.plot(best_traj[:,0,2].cpu(),label="Quarantined")

plt.plot(best_traj[:,0,3].cpu(),label="Recovered")
...
## Now NIO searches for policies that produce the largest outbreak.
loss = -torch.sum(I)
...
| Method          |    β |    δ |    γ | Total Infection |
| --------------- | ---: | ---: | ---: | --------------: |
| Random Mean     | 0.50 | 0.48 | 0.46 |            1820 |
| Defensive NIO   | 0.13 | 0.87 | 0.84 |             640 |
| Adversarial NIO | 0.88 | 0.06 | 0.08 |            2965 |
